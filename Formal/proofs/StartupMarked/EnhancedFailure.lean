import proofs.StartupMarked.EnhancedAE
...
theorem startup_enhanced_failure (hn : 4 ≤ n)
    ...
    : physicalTrajectoryLaw (by omega : 2 ≤ n) c V 1 (by linarith) (by norm_num) basal cat N
        {z | ¬enhancedMission V z} < ENNReal.ofReal (11/10000 : ℝ) := by
  ...
  have hae := startup_enhanced_ae (by omega : 2 ≤ n) c V hV basal cat hb hcatCap hfood r
    hl hr hlen huw huz hwz hsel hcat N hinitM hinitL hinitR
  have hsub : ∀ᵐ z ∂μ,z ∈ {z | ¬enhancedMission V z} → z ∈ (EC ∪ EF) ∪ EM := by
