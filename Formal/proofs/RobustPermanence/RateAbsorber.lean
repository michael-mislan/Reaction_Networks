...
structure IsRateTrajectory (r : AssemblyRates) (S : ℝ → State) (x : ℝ → ℝ) : Prop where
  positive : ∀ t, 0 ≤ t → (S t).Positive
  consumer_positive : ∀ t, 0 ≤ t → 0 < x t
  dx : ∀ t, 0 ≤ t → HasDerivAt x (rateX r (S t).z (x t)) t
...
theorem rate_eventually_resident_absorbing (r : AssemblyRates) (hr : RateBox r)
  ...
  have hW : ∀ᶠ t in atTop, (S t).z+(7/4:ℝ)*(S t).H < 384 := by
    ...
    · intro t ht
      ...
      exact rate_weighted_upper r hr _ _ _ _ _ hp.1.le (by linarith [hp.2.1])
        hp.2.1.le hp.2.2.1.le hp.2.2.2.le (hS.consumer_positive t h0).le
  ...
  have hz : ∀ᶠ t in atTop, (S t).z < 12 := by
    ...
    · intro t ht
      ...
      exact rate_z_upper r hr _ _ _ _ _ hp.1.le (by linarith [hTs t hTs',hp.2.1])
        hp.2.1.le hp.2.2.1.le hp.2.2.2.le (hS.consumer_positive t h0).le (hTw t hTw').le
...
theorem rate_eventually_consumer_upper (r : AssemblyRates) (hr : RateBox r)
    (S : ℝ → State) (x : ℝ → ℝ) (hS : IsRateTrajectory r S x) :
    ∀ᶠ t in atTop, x t < 12 := by
  obtain ⟨T,hT⟩ := eventually_atTop.1 (rate_eventually_resident_absorbing r hr S x hS)
  apply eventual_upper_of_linear_drift x (fun t => rateX r (S t).z (x t))
    (max T 0) 139 12 12 (by norm_num) (by norm_num)
  · intro t ht
    exact hS.dx t ((le_max_right _ _).trans ht)
  · intro t ht
    exact rate_X_upper r hr _ _ (hT t ((le_max_left _ _).trans ht)).2.2.1.le
      (hS.consumer_positive t ((le_max_right _ _).trans ht)).le
