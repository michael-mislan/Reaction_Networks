...
structure IsConsumerTrajectory (e : ℝ) (X : ℝ → State) (x : ℝ → ℝ) : Prop where
  positive : ∀ t, 0 ≤ t → (X t).Positive
  consumer_positive : ∀ t, 0 ≤ t → 0 < x t
  dx : ∀ t, 0 ≤ t → HasDerivAt x (x t * ((X t).z-1/2-x t)) t
...
theorem consumer_eventually_resident_absorbing (e : ℝ) (he : 0 ≤ e) (heM : e ≤ 1/50000)
    (X : ℝ → State) (x : ℝ → ℝ) (hX : IsConsumerTrajectory e X x) :
  ...
  have hW : ∀ᶠ t in atTop, (X t).z+(7/4:ℝ)*(X t).H < 384 := by
    ...
    · intro t ht
      ...
      have hfeed := mul_nonneg hp.2.2.1.le (hX.consumer_positive t h0).le
