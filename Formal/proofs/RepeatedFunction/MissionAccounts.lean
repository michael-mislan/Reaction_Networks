...
theorem physical_mission_accounts (hn : 2 ≤ n) (c : SourceMoleculeFibreConfig n)
    ...
    ∀ᵐ z ∂physicalTrajectoryLaw hn c (V : NNReal) 1 (by exact_mod_cast hV) (by norm_num)
      basal cat (foodOnlyCounts n V), twoWindowMission (V : NNReal) z →
  ...
  exact food_only_mission_net_synthesis c (V : NNReal) hv basal cat z
