import proofs.RepeatedFunction.MissionLower
...
theorem concrete_two_window_success_lower (hn : 4 ≤ n) (V : ℕ) (hV : 0 < V)
  ...
  apply physical_two_window_success_lower hn c (V : NNReal) (by exact_mod_cast hV) basal cat
...
theorem source_two_window_success_lower (hn : 4 ≤ n) (V : ℕ) (hV : 0 < V)
  ...
  · intro c hseed
    exact concrete_two_window_success_lower hn V hV hscale c basal cat hb hc hseed

/-- Every source row, kinetic mark, and physical clock is averaged. -/
def fullyAveragedTwoWindowProbability (hn : 4 ≤ n) (V : ℕ) (hV : 0 < V) (a : ℝ) : ℝ :=
...
theorem fully_averaged_two_window_lower (hn : 4 ≤ n) (V : ℕ) (hV : 0 < V)
    (hscale : 2*(n : ℝ)/markedNoiseDelta ≤ (V : ℝ)) (a : ℝ) (ha : 1 < a) :
    (sourceEmptyRowMass a n ^ 6*powerLawMoleculeGatewayHit a (sourceReactionCount n) 1)*
      (1-24*Real.exp (-(twoWindowNoiseRate*(V : ℝ)/(n : ℝ)))) ≤
  ...
  calc
    ...
    _ ≤ _ := Finset.sum_le_sum (fun u _ => source_two_window_success_lower hn V hV hscale a ha
