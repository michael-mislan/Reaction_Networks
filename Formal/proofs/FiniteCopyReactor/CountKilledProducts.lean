...
def massRegion (B : ℕ) (N : Counts) : Prop := reactorMass N ≤ B

def massKilledMultiplier (B k : ℕ)
    (h : Finset.Iic k → JumpState Counts CompetitionChannel)
    (y : JumpState Counts CompetitionChannel) : ℝ≥0∞ :=
...
theorem massKilledProduct_eq (B K : ℕ)
    (z : ℕ → JumpState Counts CompetitionChannel)
...
theorem massKilledMultiplier_measurable (B k : ℕ) :
    Measurable (fun p : (Finset.Iic k → JumpState Counts CompetitionChannel) ×
      JumpState Counts CompetitionChannel => massKilledMultiplier B k p.1 p.2) := by
  have hs : MeasurableSet {N : Counts | massRegion B N} :=
...
theorem physical_killed_product_bound (V r d : ℝ) (hV : 0 < V) (hr : 0 ≤ r) (hd : 0 ≤ d)
    (N : Counts) (B : ℕ) :
    ∃ q : ℝ, 1 ≤ q ∧ ∀ K,
      (∫⁻ z, trajectoryProduct (massKilledMultiplier B) K z ∂reactorTrajectory V r d hV hr hd N) ≤
        (ENNReal.ofReal (q/(q+1)))^K := by
  obtain ⟨q, hq, hb⟩ := reactor_locally_bounded V r d hV hr hd B
  refine ⟨q, hq, ?_⟩
  intro K
  apply predictable_product_integral_le (reactorTrajectory V r d hV hr hd N)
  ...
  · simp only [massKilledMultiplier, if_pos hs]
    exact (jumpState_wait_contraction reactorNext (reactorRate V r d)
      (reactor_rate_nonneg V r d hV hr hd) (reactor_total_pos V r d hV hr hd)
