...
def LowFreeActive (V : ℕ) : Set (BoxCounts V) :=
  {X | residenceActive V X ∧ (boxCounts X 2:ℝ) ≤ (V:ℝ)/1000}

theorem residence_free_indicator (V : ℕ) (N : BoxCounts V) :
    FiniteKernel.eventIndicator (LowFreeActive V) N ≤
      Real.exp ((V:ℝ)/1000000000)*residencePhaseTest V (1/1000000) (phaseWeights (3000*(V:ℝ)) 0) N := by
  by_cases h : N ∈ LowFreeActive V
...
theorem residence_free_window (V : ℕ) (r d : ℝ)
    ...
    (residenceKernel V r d hV (by linarith) hr' hd hd').steps n
      (FiniteKernel.eventIndicator (LowFreeActive V)) N ≤ Real.exp (-(V:ℝ)/10000000000) := by
  let R := residenceKernel V r d hV (by linarith) hr' hd hd'
  have hi := R.steps_mono (residence_free_indicator V) n N
  rw [R.steps_scale] at hi
  have ht := residence_phase_steps V r d (1/1000000) hV hr hr' hd hd'
    (by norm_num) (by norm_num) n N
  have hinit : residencePhaseTest V (1/1000000) (phaseWeights (3000*(V:ℝ)) n) N ≤
...
theorem residence_free_after_burnin (V : ℕ) (r d : ℝ)
    (hV : 0 < (V:ℝ)) (hr : 19 ≤ r) (hr' : r ≤ 21) (hd : 0 ≤ d) (hd' : d ≤ 1/25)
    (n : ℕ) (hn : 90*V ≤ n) (N : BoxCounts V) :
    (residenceKernel V r d hV (by linarith) hr' hd hd').steps n
      (FiniteKernel.eventIndicator (LowFreeActive V)) N ≤ Real.exp (-(V:ℝ)/10000000000) := by
  let R := residenceKernel V r d hV (by linarith) hr' hd hd'
  have hh := R.steps_mono
    (residence_free_window V r d hV hr hr' hd hd' (90*V) (by omega) (by omega)) (n-90*V) N
