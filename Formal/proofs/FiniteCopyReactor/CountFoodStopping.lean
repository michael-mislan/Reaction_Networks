...
import proofs.FiniteCopyReactor.CountStoppedProducts
...
theorem foodHistorySum_measurable (k : ℕ) : Measurable (foodHistorySum k) := by
...
theorem foodHistorySum_stop_measurable (b k : ℕ) : MeasurableSet {h | b ≤ foodHistorySum k h} :=
