...
def pulseCycle (N : Counts) (V : ℕ) (p : Intervention) (hN : Restart V N)
    (r d : ℝ) (hV : 0 < (V:ℝ)) (hr : 0 ≤ r) (hr' : r ≤ 21) (hd : 0 ≤ d) (hd' : d ≤ 1/25)
    (f : BoxCounts V × ReactorCounters → ℝ) : ℝ :=
  ∑ o,pulseMass N p o*jointCycle V r d hV hr hr' hd hd' f (postPulseBox N V p hN o)
...
theorem pulse_cycle_failure_bound (N : Counts) (V : ℕ) (p : Intervention) (hN : Restart V N)
    (hlarge : 1000000 ≤ V) (r d : ℝ) (hV : 0 < (V:ℝ))
    (hr : 19 ≤ r) (hr' : r ≤ 21) (hd : 0 ≤ d) (hd' : d ≤ 1/25) :
    pulseCycle N V p hN r d hV (by linarith) hr' hd hd'
      (FiniteKernel.eventIndicator {X | ¬CycleSuccess V X}) ≤ oneCycleError V := by
  let E := BadPulseStock N V p ∪ BadPulseMaterial N V p
  have hp (o : PulseOutcome N) :
      jointCycle V r d hV (by linarith) hr' hd hd' (FiniteKernel.eventIndicator {X | ¬CycleSuccess V X})
    ...
    · rw [if_neg ho,zero_add]
      ...
      exact prepared_success_failure_bound V r d hV hlarge hr hr' hd hd' _ hs hm _ _
