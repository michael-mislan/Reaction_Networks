...
import proofs.DegradationControl.External.SchauderFixedPoint
...
/-- Brouwer/Schauder existence for an entrywise-positive finite real matrix. -/
theorem positiveMatrix_exists_normalizedPositive_eigenvector
    [Fintype ι] [DecidableEq ι] [Nonempty ι]
    {P : Matrix ι ι ℝ} (hP : IsPositiveMatrix P) :
    ∃ rho : ℝ, 0 < rho ∧ ∃ v : ι → ℝ,
      NormalizedPositive v ∧ P.mulVec v = rho • v := by
  obtain ⟨v, hvS, hvfix⟩ := Submission.schauder_fixed_point
    (isCompact_stdSimplex ℝ ι) (convex_stdSimplex ℝ ι)
    ⟨_, ite_eq_mem_stdSimplex ℝ (Classical.arbitrary ι)⟩ (normalizedMulVec P)
    (continuous_normalizedMulVecOn hP)
    (fun _ hx ↦ normalizedMulVec_mem_stdSimplex hP hx)
  let rho := totalOutput P v
  have hrho : 0 < rho := totalOutput_pos_of_positive_of_mem_stdSimplex hP hvS
  have hvpos : StrictlyPositive v := by
    intro i
    rw [← hvfix]
    exact div_pos (mulVec_pos_of_positive_of_mem_stdSimplex hP hvS i) hrho
  refine ⟨rho, hrho, v, ⟨hvpos, hvS.2⟩, ?_⟩
  funext i
  have hi := congrFun hvfix i
  simp only [normalizedMulVec] at hi
  simp only [Pi.smul_apply, smul_eq_mul]
  change P.mulVec v i / rho = v i at hi
  have hmul := (div_eq_iff (ne_of_gt hrho)).mp hi
