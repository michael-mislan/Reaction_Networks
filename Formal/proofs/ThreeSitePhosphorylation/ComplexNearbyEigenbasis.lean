import proofs.ThreeSitePhosphorylation.SimpleComplexEigenpair
import Mathlib.Analysis.Normed.Ring.Units
import Mathlib.Order.Filter.Finite
import Mathlib.Tactic.Choose

namespace ThreeSitePhosphorylation.ComplexNearbyEigenbasis
...
def columnMap (b : Module.Basis ι ℂ E) (v : ι → E) : E →L[ℂ] E :=
  ∑ i, (b.coord i).toContinuousLinearMap.smulRight (v i)

theorem columnMap_basis (b : Module.Basis ι ℂ E) (v : ι → E) (i : ι) :
    columnMap b v (b i)=v i := by
  simp [columnMap,ContinuousLinearMap.sum_apply,Module.Basis.coord_apply,Finsupp.single_apply]

theorem columnMap_self (b : Module.Basis ι ℂ E) : columnMap b b=1 := by
...
theorem columnMap_continuousAt (b : Module.Basis ι ℂ E) (v : P → ι → E)
  ...
  have hterm (i : ι) : ContinuousAt
    ...
    (ContinuousLinearMap.smulRightL ℂ E E (b.coord i).toContinuousLinearMap).continuous.continuousAt.comp (hv i)
  have hs (s : Finset ι) : ContinuousAt
      (fun a => ∑ i ∈ s, (b.coord i).toContinuousLinearMap.smulRight (v a i)) 0 := by
    induction s using Finset.induction_on with
    | empty => simpa using (continuousAt_const : ContinuousAt (fun _ : P => (0 : E →L[ℂ] E)) 0)
    | @insert i s hi ih => simpa only [Finset.sum_insert hi] using (hterm i).add ih
  exact hs Finset.univ

omit [NormedSpace ℝ P] [CompleteSpace P] in
theorem eventually_columnMap_isUnit (b : Module.Basis ι ℂ E) (v : P → ι → E)
    (hv : ∀ i, ContinuousAt (fun a => v a i) 0) (hzero : v 0=b) :
    ∀ᶠ a in 𝓝 (0:P), IsUnit (columnMap b (v a)) := by
...
def inverseCoordinate (b : Module.Basis ι ℂ E) (v : ι → E) (i : ι) : E →L[ℂ] ℂ :=
  (b.coord i).toContinuousLinearMap.comp (Ring.inverse (columnMap b v))

theorem inverseCoordinate_self (b : Module.Basis ι ℂ E) (i : ι) :
...
omit [NormedSpace ℝ P] [CompleteSpace P] in
theorem inverseCoordinate_continuousAt (b : Module.Basis ι ℂ E) (v : P → ι → E)
    (hv : ∀ i, ContinuousAt (fun a => v a i) 0) (hzero : v 0=b) (i : ι) :
    ContinuousAt (fun a => inverseCoordinate b (v a) i) 0 := by
  haveI : CompleteSpace E := FiniteDimensional.complete ℂ E
  have hi : ContinuousAt (Ring.inverse : (E →L[ℂ] E) → (E →L[ℂ] E))
      (columnMap b (v 0)) := by
    rw [hzero,columnMap_self]
    simpa using NormedRing.inverse_continuousAt (1 : (E →L[ℂ] E)ˣ)
  have hcomp : ContinuousAt (fun a : P => Ring.inverse (columnMap b (v a))) 0 :=
    ContinuousAt.comp (f := fun a : P => columnMap b (v a)) hi (columnMap_continuousAt b v hv)
  exact ContinuousAt.comp (f := fun a : P => Ring.inverse (columnMap b (v a)))
    (ContinuousLinearMap.compL ℂ E E ℂ (b.coord i).toContinuousLinearMap).continuous.continuousAt hcomp

theorem inverseCoordinate_columns (b : Module.Basis ι ℂ E) (v : ι → E)
...
theorem exists_basis_of_columnMap_isUnit (b : Module.Basis ι ℂ E) (v : ι → E)
    (hu : IsUnit (columnMap b v)) :
    ∃ c : Module.Basis ι ℂ E, (∀ i, c i=v i) ∧
...
theorem inverseCoordinate_left_eigen (b : Module.Basis ι ℂ E) (v : ι → E)
    (hu : IsUnit (columnMap b v)) (A : E →L[ℂ] E) (μ : ι → ℂ)
...
theorem nearby_eigenbasis (b : Module.Basis ι ℂ E) (v : P → ι → E)
    (hv : ∀ i, ContinuousAt (fun a => v a i) 0) (hzero : v 0=b)
    (A : P → E →L[ℂ] E) (μ : P → ι → ℂ)
    (he : ∀ᶠ a in 𝓝 (0:P), ∀ i, A a (v a i)=μ a i • v a i) :
    (∀ i, ContinuousAt (fun a => inverseCoordinate b (v a) i) 0) ∧
    ∀ᶠ a in 𝓝 (0:P), ∃ c : Module.Basis ι ℂ E,
      (∀ i, c i=v a i) ∧
...
variable [NormedSpace ℝ E] [IsScalarTower ℝ ℂ E] [FiniteDimensional ℝ E]

/-- A real-smooth complex operator family with a supplied simple complex eigenbasis has
...
theorem smooth_eigenbasis (A : P → E →L[ℂ] E)
  ...
  refine ⟨fun a i => μ i a,fun a i => v i a,hμ,hv,funext hμ0,hzero,huniq,hb.1,?_⟩
  filter_upwards [hall,hb.2] with a ha hba
  exact ⟨ha,hba⟩

end
end ThreeSitePhosphorylation.ComplexNearbyEigenbasis
