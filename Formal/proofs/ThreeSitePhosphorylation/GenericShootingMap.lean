...
/-! Shooting residuals and actual line derivatives in a real Banach space.
No shooting derivative invertibility or periodic-family existence is assumed. -/
namespace ThreeSitePhosphorylation.GenericShootingMap
noncomputable section
open scoped Topology

abbrev FlowData (E : Type*) := (ℝ × (ℝ × ℝ)) × E
abbrev ShootingState (E : Type*) := E × ℂ
abbrev SourcePath (E : Type*) [TopologicalSpace E] := ContinuousPath E

variable {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E] [CompleteSpace E]

def shootingArgument (p : ℝ × ShootingState E) : FlowData E :=
  ((p.1,(p.2.2.re,p.2.2.im)),p.2.1)

def shootingMap (ψ : FlowData E → SourcePath E) (L : E →L[ℝ] ℂ)
    (x : E) (p : ℝ × ShootingState E) : ShootingState E :=
  ((ψ (shootingArgument p)) ⟨1,by norm_num⟩-p.2.1,L (p.2.1-x))

omit [CompleteSpace E] in
theorem shootingArgument_smooth : ContDiff ℝ ⊤ (shootingArgument (E := E)) := by
  unfold shootingArgument
  have hr : ContDiff ℝ ⊤ (fun p : ℝ × ShootingState E => p.2.2.re) :=
    Complex.reCLM.contDiff.comp (f := fun p : ℝ × ShootingState E => p.2.2) contDiff_snd.snd
  have hi : ContDiff ℝ ⊤ (fun p : ℝ × ShootingState E => p.2.2.im) :=
    Complex.imCLM.contDiff.comp (f := fun p : ℝ × ShootingState E => p.2.2) contDiff_snd.snd
  exact (contDiff_fst.prodMk (hr.prodMk hi)).prodMk contDiff_snd.fst

omit [CompleteSpace E] in
theorem shootingMap_smooth (ψ : FlowData E → SourcePath E) (L : E →L[ℝ] ℂ)
    (x : E) (p : ℝ × ShootingState E)
    (hψ : ContDiffAt ℝ ⊤ ψ (shootingArgument p)) :
    ContDiffAt ℝ ⊤ (shootingMap ψ L x) p := by
  have he := (ContinuousMap.evalCLM (R := ℝ) (M := E) (⟨1,by norm_num⟩:UnitTime)).contDiff (n := ⊤)
...
omit [CompleteSpace E] in
theorem shootingMap_zero (ψ : FlowData E → SourcePath E) (L : E →L[ℝ] ℂ)
    (x : E) (r T : ℝ)
...
omit [CompleteSpace E] in
theorem shootingMap_direction (ψ : FlowData E → SourcePath E) (L : E →L[ℝ] ℂ)
    (x dx : E) (r T dr dT : ℝ) (q : SourcePath E)
  ...
  have he := (ContinuousMap.evalCLM (R := ℝ) (M := E)
...
omit [CompleteSpace E] in
theorem flowLine_derivative (r T dr dT : ℝ) (x dx : E) :
    HasDerivAt (fun s : ℝ => ((0,(r+s*dr,T+s*dT)),x+s • dx) : ℝ → FlowData E)
      ((0,(dr,dT)),dx) 0 := by
...
omit [CompleteSpace E] in
theorem shootingLine_derivative (r T : ℝ) (x : E) (d : ShootingState E) :
    HasDerivAt (fun s : ℝ => (0,(x+s • d.1,
      ((r+s*d.2.re:ℝ):ℂ)+Complex.I*((T+s*d.2.im:ℝ):ℂ))) : ℝ → ℝ × ShootingState E)
      (0,d) 0 := by
...
end ThreeSitePhosphorylation.GenericShootingMap
