...
namespace ThreeSitePhosphorylation.SimpleEigenpair
...
theorem basis_coordinate_eigen (A : E →ₗ[ℝ] E) (b : Module.Basis ι ℝ E)
    (eig : ι → ℝ) (he : ∀ i, A (b i)=eig i • b i) (i : ι) (v : E) :
...
def eigenpairLinear (A : E →ₗ[ℝ] E) (b : Module.Basis ι ℝ E)
    (eig : ℝ) (i : ι) : (E × ℝ) →ₗ[ℝ] (E × ℝ) where
...
a supplied real eigenbasis. This is structural, with no determinant expansion. -/
theorem eigenpairLinear_bijective (A : E →ₗ[ℝ] E) (b : Module.Basis ι ℝ E)
    (eig : ι → ℝ) (he : ∀ i, A (b i)=eig i • b i)
    (hi : Function.Injective eig) (i : ι) :
  ...
  have hk : ∀ z, eigenpairLinear A b (eig i) i z=0 → z=0 := by
    ...
    have hv : z.1=0 := by
      ...
      · have hh := congrArg (b.coord j) hA
        ...
        have hd : eig j-eig i ≠ 0 := sub_ne_zero.mpr (fun h => hji (hi h))
...
def eigenpairResidual (A : ℝ → E →L[ℝ] E) (b : Module.Basis ι ℝ E)
    (i : ι) (d : ℝ × (E × ℝ)) : E × ℝ :=
  (A d.1 d.2.1-d.2.2 • d.2.1,b.coord i d.2.1-1)

omit [Fintype ι] [DecidableEq ι] in
theorem eigenpairResidual_smooth (A : ℝ → E →L[ℝ] E)
    (hA : ContDiffAt ℝ ⊤ A 0) (b : Module.Basis ι ℝ E) (i : ι) (z : E × ℝ) :
    ContDiffAt ℝ ⊤ (eigenpairResidual A b i) (0,z) := by
  have ha : ContDiffAt ℝ ⊤ (fun d : ℝ × (E × ℝ) => A d.1) (0,z) :=
    ContDiffAt.comp (f := fun d : ℝ × (E × ℝ) => d.1) (g := A) (0,z) hA contDiffAt_fst
  have hc : ContDiffAt ℝ ⊤ (fun d : ℝ × (E × ℝ) => b.coord i d.2.1) (0,z) :=
    ContDiffAt.comp (f := fun d : ℝ × (E × ℝ) => d.2.1)
      (g := (b.coord i).toContinuousLinearMap) (0,z)
      (b.coord i).toContinuousLinearMap.contDiff.contDiffAt (contDiffAt_snd.fst)
  exact ((ha.clm_apply contDiffAt_snd.fst).sub
    (contDiffAt_snd.snd.smul contDiffAt_snd.fst)).prodMk (hc.sub contDiffAt_const)

omit [Fintype ι] [DecidableEq ι] in
theorem eigenpairResidual_slice_derivative (A : ℝ → E →L[ℝ] E)
    (b : Module.Basis ι ℝ E) (eig : ℝ) (i : ι) :
    HasFDerivAt (fun z : E × ℝ => eigenpairResidual A b i (0,z))
      (eigenpairLinear (A 0).toLinearMap b eig i).toContinuousLinearMap (b i,eig) := by
  let fstL := ContinuousLinearMap.fst ℝ E ℝ
  let sndL := ContinuousLinearMap.snd ℝ E ℝ
  ...
  have hc := ((b.coord i).toContinuousLinearMap.hasFDerivAt.comp (b i,eig)
    fstL.hasFDerivAt).sub_const (1:ℝ)
...
omit [Fintype ι] in
/-- A simple eigenvalue from a supplied real eigenbasis continues to an
actual normalized smooth real eigenpair. No determinant expansion is used. -/
theorem smooth_normalized_eigenpair (A : ℝ → E →L[ℝ] E)
    (hA : ContDiffAt ℝ ⊤ A 0) (b : Module.Basis ι ℝ E)
    (eig : ι → ℝ) (he : ∀ j, A 0 (b j)=eig j • b j)
    (hi : Function.Injective eig) (i : ι) :
    ∃ (μ : ℝ → ℝ) (v : ℝ → E),
      ContDiffAt ℝ ⊤ μ 0 ∧ ContDiffAt ℝ ⊤ v 0 ∧
      μ 0=eig i ∧ v 0=b i ∧
      ∀ᶠ a in 𝓝 (0:ℝ), A a (v a)=μ a • v a ∧ b.coord i (v a)=1 := by
  let F := eigenpairResidual A b i
  let z0 : E × ℝ := (b i,eig i)
  let L := (eigenpairLinear (A 0).toLinearMap b (eig i) i).toContinuousLinearMap
  have hs : ContDiffAt ℝ ⊤ F (0,z0) := eigenpairResidual_smooth A hA b i z0
  have hpart : fderiv ℝ F (0,z0) ∘L ContinuousLinearMap.inr ℝ ℝ (E × ℝ)=L := by
    have hd : HasFDerivAt (fun z : E × ℝ => ((0:ℝ),z))
        (ContinuousLinearMap.inr ℝ ℝ (E × ℝ)) z0 := by
      convert (hasFDerivAt_const (0:ℝ) z0).prodMk (hasFDerivAt_id z0) using 1
    have hc := (hs.differentiableAt (by simp)).hasFDerivAt.comp z0 hd
    exact hc.unique (eigenpairResidual_slice_derivative A b (eig i) i)
  have hInv : (fderiv ℝ F (0,z0) ∘L ContinuousLinearMap.inr ℝ ℝ (E × ℝ)).IsInvertible := by
    rw [hpart]
    have hb := eigenpairLinear_bijective (A 0).toLinearMap b eig he hi i
    obtain ⟨u,hu⟩ := ContinuousLinearMap.isUnit_iff_bijective.mpr (show Function.Bijective L from hb)
    exact ⟨ContinuousLinearEquiv.unitsEquiv ℝ (E × ℝ) u,hu⟩
  ...
  have hge : ∀ᶠ a in 𝓝 (0:ℝ), F (a,g a)=0 := by
...
end ThreeSitePhosphorylation.SimpleEigenpair
