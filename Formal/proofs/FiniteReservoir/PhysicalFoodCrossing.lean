import proofs.FiniteReservoir.PhysicalFoodStopping
...
def foodCrossingEvent (b : ℕ) (T : ℝ) :
    Set (ℕ → JumpState (CountState M) CompetitionChannel) :=
  ⋃ K, {z | foodCrossingBy b T K z}

theorem food_crossing_weighted_probability (M : ℕ) (p : Parameters M) (V : ℝ) (hV : 0 < V) 
    (N : CountState M) (b K : ℕ) (T : ℝ) :
    ENNReal.ofReal ((2 : ℝ)^b * Real.exp (-(2*V)*T)) *
      reactorTrajectory M p V hV N {z | foodCrossingBy b T K z} ≤ 1 := by
  let a := ENNReal.ofReal ((2 : ℝ)^b * Real.exp (-(2*V)*T))
  have ht := stopped_physical_product_tail M p V hV N
    (foodBudgetStop b) (foodHistorySum_stop_measurable b) K a
  have hsub : {z : ℕ → JumpState (CountState M) CompetitionChannel | foodCrossingBy b T K z} ⊆
...
theorem food_crossing_probability (M : ℕ) (p : Parameters M) (V : ℝ) (hV : 0 < V) 
    (N : CountState M) (b K : ℕ) (T : ℝ) :
    reactorTrajectory M p V hV N {z | foodCrossingBy b T K z} ≤
  ...
  have ht := food_crossing_weighted_probability M p V hV N b K T
  calc
    _ = B*(a*reactorTrajectory M p V hV N {z | foodCrossingBy b T K z}) := by
      rw [← mul_assoc, ha, one_mul]
    _ ≤ B*1 := mul_le_mul_right ht B
    _ = B := mul_one _

theorem food_crossing_any_jump_probability (M : ℕ) (p : Parameters M) (V : ℝ) (hV : 0 < V) 
    (N : CountState M) (b : ℕ) (T : ℝ) :
    reactorTrajectory M p V hV N (foodCrossingEvent b T) ≤
      ENNReal.ofReal (Real.exp ((2*V)*T)/(2 : ℝ)^b) := by
  have hm : Monotone (fun K => {z : ℕ → JumpState (CountState M) CompetitionChannel | foodCrossingBy b T K z}) := by
    intro K L hKL z hz
    obtain ⟨j, hj, hp, hc, ht⟩ := hz
    exact ⟨j, hj.trans hKL, hp, hc, ht⟩
  rw [foodCrossingEvent, hm.measure_iUnion]
  exact iSup_le (fun K => food_crossing_probability M p V hV N b K T)
