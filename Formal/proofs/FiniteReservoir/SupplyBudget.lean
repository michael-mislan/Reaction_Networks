import proofs.FiniteReservoir.SupplyKernel
...
def supplyCycle (k : SupplyKind) (V M : ℕ) (p : Parameters M) (hV : 0 < (V:ℝ))
    
    (f : BoxState V M → ℝ → ℝ) (N : BoxState V M) (z : ℝ) : ℝ :=
  threeStage (supplyKernel false k V M p hV)
    (supplyKernel true k V M p hV) (supplyKernel true k V M p hV)
    ((8250:ℝ≥0)*V) ((750:ℝ≥0)*V) ((3000:ℝ≥0)*V) f N z

theorem supply_cycle_upper (k : SupplyKind) (V M : ℕ) (p : Parameters M) (hV : 0 < (V:ℝ))
    
    (s K : ℝ) (hs : 0 ≤ s) (N : BoxState V M) (z : ℝ) :
    supplyCycle k V M p hV (MarkedKernel.eventIndicator {a | K ≤ a.2}) N z ≤
  ...
  have hp : 0 ≤ 1+supplyRateCap k/3000*(Real.exp s-1) := by
    ...
    positivity
  have h := three_stage_upper (supplyKernel false k V M p hV)
    (supplyKernel true k V M p hV) (supplyKernel true k V M p hV)
    ((8250:ℝ≥0)*V) ((750:ℝ≥0)*V) ((3000:ℝ≥0)*V) s
    (1+supplyRateCap k/3000*(Real.exp s-1)) K hs hp
    (supply_exponential_step false k V M p hV s hs)
    (supply_exponential_step true k V M p hV s hs)
    (supply_exponential_step true k V M p hV s hs) N z
  apply h.trans_eq
  congr 1
...
theorem food_cycle_upper (k : SupplyKind) (hk : k=.foodU ∨ k=.foodW)
    (V M : ℕ) (p : Parameters M) (hV : 0 < (V:ℝ))
    
    (N : BoxState V M) (z : ℝ) (hz : z ≤ (151/200)*(V:ℝ)) :
    supplyCycle k V M p hV
  ...
  have h := supply_cycle_upper k V M p hV (1/50) (5*(V:ℝ)) (by norm_num) N z
  apply h.trans
...
theorem gross_cycle_upper (V M : ℕ) (p : Parameters M) (hV : 0 < (V:ℝ))
     (N : BoxState V M) :
    supplyCycle .gross V M p hV
      (MarkedKernel.eventIndicator {a | (V:ℝ)/5 ≤ a.2}) N 0 ≤ Real.exp (-(V:ℝ)/2000) := by
  have h := supply_cycle_upper .gross V M p hV (1/20) ((V:ℝ)/5) (by norm_num) N 0
  apply h.trans
  apply Real.exp_le_exp.mpr
  dsimp [supplyRateCap]
