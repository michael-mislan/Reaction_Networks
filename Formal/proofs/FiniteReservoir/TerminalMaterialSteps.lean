import proofs.FiniteReservoir.TerminalMaterial

namespace FiniteReservoir
noncomputable section
open ProductiveRecovery RandomViability.Binding FiniteCopy Classical

theorem material_compensated_steps (V M : ℕ) (side : Bool) (p : Parameters M) (s q : ℝ) (hV : 0 < (V:ℝ)) (hs : |s| ≤ 1/100) (hq : 0 < q) (hq1 : 1 ≤ q)
    (hbound : ∀ N, (materialModel V M p hV).total N ≤ q) (n : ℕ) (N : BoxState V M) :
    ((materialModel V M p hV).uniformize q hq hbound).steps n (materialCompensated V M side s) N ≤
      materialCompensated V M side ((1-1/q)^n*s) N := by
  let P := (materialModel V M p hV).uniformize q hq hbound
  ...
  | succ n ih =>
    ...
    have hh := material_compensated_step V M side p ((1-1/q)^n*s) q hV hsn hq hq1 hbound N
    have he : (1-1/q)*((1-1/q)^n*s) = (1-1/q)^(n+1)*s := by rw [pow_succ]; ring
    rw [he] at hh
    exact hm.trans hh

theorem material_test_steps (V M : ℕ) (side : Bool) (p : Parameters M) (s q : ℝ) (hV : 0 < (V:ℝ)) (hs : |s| ≤ 1/100) (hq : 0 < q) (hq1 : 1 ≤ q)
    (hbound : ∀ N, (materialModel V M p hV).total N ≤ q) (n : ℕ) (N : BoxState V M) :
    ((materialModel V M p hV).uniformize q hq hbound).steps n (materialTest V M side s) N ≤
      Real.exp (2*s^2*(V:ℝ))*materialTest V M side ((1-1/q)^n*s) N := by
  have h := material_compensated_steps V M side p s q hV hs hq hq1 hbound n N
  ...
  have ht : 0 ≤ materialTest V M side ((1-1/q)^n*s) N := by
    unfold materialTest
    split_ifs
    · positivity
    · exact le_rfl
