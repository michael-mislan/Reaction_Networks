import proofs.FiniteReservoir.FreeShortfall
...
def freeCollectionFailure (V M : ℕ) (p : Parameters M) (hV : 0 < (V:ℝ))
     (N : BoxState V M) : ℝ :=
...
theorem free_precollection_bound (V M : ℕ) (p : Parameters M) (hV : 0 < (V:ℝ)) (hlarge : 1000000 ≤ V)
  ...
  have hh := poisson_sequence_cutoff ((750:ℝ≥0)*V) (fun m => R.steps m F N)
    ...
    (fun m hm => free_shortfall_discrete V M p hV hlarge m n hm hn N)
  apply hh.trans
...
theorem free_collection_probability (V M : ℕ) (p : Parameters M) (hV : 0 < (V:ℝ)) (hlarge : 1000000 ≤ V)
     (N : BoxState V M) :
    freeCollectionFailure V M p hV N ≤ freeCollectionError V := by
  let R := residenceKernel V M p hV
  let P := residenceMarked V M p hV
  ...
  have hh := poisson_sequence_cutoff ((3000:ℝ≥0)*V)
    ...
    (fun n hn => free_precollection_bound V M p hV hlarge n hn N)
  apply hh.trans
