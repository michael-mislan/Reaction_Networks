...
import proofs.TypeIIL.WeightedTransfer

namespace TypeIIL

open TypeII3

/-- Reduced sector allowing a signed linear coefficient `B`.  Weighted cycle
steps preserve the positive cross determinant `A*b+a*B`, which is the exact
monotonicity invariant used by the transfer map. -/
structure GeneralSectorParams where
  A : ℝ
  B : ℝ
  lam : ℝ
  a : ℝ
...
def generalReducedF (q : GeneralSectorParams)
    (xPrev x xNext : ℝ) : ℝ :=
  q.A * xPrev - q.B * x -
    q.lam * (q.a * xPrev + q.b * x) * (q.e * x + q.f * xNext) ^ q.m

theorem general_sector_comparison_with_gain
    (q : GeneralSectorParams) (hef : q.f < q.e)
    {xPrev x xNext yPrev y yNext : ℝ}
    (hxPrev : 0 < xPrev) (hx : 0 < x) (hxNext : 0 < xNext)
    (hyPrev : 0 < yPrev) (hy : 0 < y) (hyNext : 0 < yNext)
    (hFx : generalReducedF q xPrev x xNext = 0)
    (hFy : generalReducedF q yPrev y yNext = 0) :
  ...
  have hrearrX :
    ...
    unfold generalReducedF at hFx
    dsimp [tx]
    linear_combination hFx
  have hrearrY :
      (q.A - q.lam * ty ^ q.m * q.a) * yPrev =
        (q.B + q.lam * q.b * ty ^ q.m) * y := by
    unfold generalReducedF at hFy
    dsimp [ty]
    linear_combination hFy
  have hdenX : 0 < q.A - q.lam * tx ^ q.m * q.a :=
    denominator_pos_of_root_cross q.a_pos q.b_pos q.cross_pos hxPrev hx hrearrX
  have hdenY : 0 < q.A - q.lam * ty ^ q.m * q.a :=
    denominator_pos_of_root_cross q.a_pos q.b_pos q.cross_pos hyPrev hy hrearrY
  have hratioX :
  ...
  have hK : 0 < K := by
    dsimp [K]
    exact transferGain_pos_cross q.lam_pos q.cross_pos htx hty q.m_pos hdenX hdenY
  have hdiff : xPrev / x - yPrev / y = K * (tx - ty) := by
    rw [hratioX, hratioY]
    simpa [K] using
      (transfer_difference (A := q.A) (B := q.B) (lam := q.lam)
        (a := q.a) (b := q.b) (r := tx) (s := ty) (m := q.m)
        (ne_of_gt hdenX) (ne_of_gt hdenY))
  have hdiff' : xPrev / x - yPrev / y =
  ...
  simpa [u, v] using comparison_equation hx hyPrev hy hyNext hdiff'

structure GeneralCyclicReducedData (l : ℕ) where
  next : Fin l ≃ Fin l
  next_singleCycle : IsSingleCycle next
  q : Fin l → GeneralSectorParams
  response_gain : ∀ i, (q i).f < (q i).e

def IsGeneralCyclicReducedRoot (D : GeneralCyclicReducedData l)
    (x : Fin l → ℝ) : Prop :=
  ∀ i, generalReducedF (D.q i) (x (D.next.symm i)) (x i) (x (D.next i)) = 0

theorem general_cyclic_reduced_unistationarity
    [NeZero l] (D : GeneralCyclicReducedData l) (x y : Fin l → ℝ)
    (hx : ∀ i, 0 < x i) (hy : ∀ i, 0 < y i)
    (hX : IsGeneralCyclicReducedRoot D x)
    (hY : IsGeneralCyclicReducedRoot D y) : x = y := by
  classical
  have hsector : ∀ i, ∃ u v : ℝ, 0 < u ∧ 0 < v ∧
    ...
    intro i
    exact general_sector_comparison_with_gain (D.q i) (D.response_gain i)
