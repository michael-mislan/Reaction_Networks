...
theorem reservoir_z_lower_drift (e A B z H x : ℝ)
    (hA : 1 ≤ A) (hB : B ≤ 34) (hz : 0 ≤ z) (hz' : z ≤ 12)
    (hH : 0 ≤ H) (hx : x ≤ 48) :
    1-150*z ≤ fZ (flagshipRates e) A B z H-z*x := by
  ...
  nlinarith only [hA,hBz,hxz,hzz,hH,hz]

theorem reservoir_loaded_resident_floors (e : ℝ) (he : 0 ≤ e) (he' : e ≤ 1/50000)
    (S : ℝ → State) (x : ℝ → ℝ) (hS : IsLoadedResidentTrajectory e S x)
    (hxu : ∀ᶠ t in atTop, x t < 48) :
    ∀ᶠ t in atTop, 1 < (S t).A ∧ 2 < (S t).B ∧
      1/300 < (S t).z ∧ 1/100 < (S t).H := by
  ...
  have hHlow : ∀ᶠ t in atTop, -(S t).H < -(1/100) := by
    apply eventual_upper_of_linear_drift (fun t => -(S t).H)
      ...
      (max TH 0) (-(16/300)) (20001/10000) (-(1/100)) (by norm_num) (by norm_num)
    · intro t ht
