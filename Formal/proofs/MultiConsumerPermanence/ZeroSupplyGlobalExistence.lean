...
theorem zero_supply_positive_global_vector (n : ℕ) (e : ℝ)
  ...
  have hQR : Q ≤ R := by
    ...
    dsimp [R]
    linarith
  obtain ⟨b,X,hb,hbone,hX0,hXd⟩ := reservoir_extension_solution n e 0 0 R (by linarith) x₀
  have hinit : ∀ i, 0 < X 0 i := by simpa only [hX0] using hx₀
  have hnonneg := reservoir_extension_nonnegative e 0 0 he (by norm_num) b (fun x => (hb x).1) X
    (fun i => (hinit i).le) hXd
  have hpart : ∀ t, 0 ≤ t → reservoirPart (X t) = X t := by
    intro t ht
    funext i
    exact max_eq_right (hnonneg t ht i)
  have hscaled : ∀ t, 0 ≤ t → HasDerivAt X (b (X t) • reservoirField e 0 0 (X t)) t := by
    intro t ht
    simpa only [hpart t ht] using hXd t
  have hs := reservoir_extension_total_bound e 0 0 S he he' hS b (fun x => (hb x).1) X hnonneg
    (by simpa only [hX0] using le_max_left (x₀ (.inl 0)+x₀ (.inl 1)) 34) hscaled
  have hw := reservoir_extension_weighted_bound e 0 0 S W (le_max_right _ _) b (fun x => (hb x).1)
    X hnonneg hs (by simpa only [hX0] using
      (le_max_left (x₀ (.inl 2)+(7/4:ℝ)*x₀ (.inl 3)) ((7/2)*(S+76)))) hscaled
  have hq := reservoir_extension_zero_supply_bound e Q b (fun x => (hb x).1) X hnonneg
    (by simpa only [hX0] using (le_refl Q)) hscaled
  ...
  have hdX : ∀ t, 0 ≤ t → HasDerivAt X (reservoirField e 0 0 (X t)) t := by
    intro t ht
    simpa only [hbone _ (hnorm t ht),one_smul] using hscaled t ht
  have hpos : ∀ t, 0 ≤ t → ∀ i, 0 < X t i := by
    intro t ht i
    exact positive_of_linear_lower (fun t => X t i) (fun t => reservoirField e 0 0 (X t) i)
      (31+0+((n:ℝ)+11)*R^2) (fun s hs => hasDerivAt_pi.1 (hdX s hs) i) (hinit i)
      (fun s hs => reservoir_field_linear_lower e 0 R he he' (by norm_num) hR (X s)
        (hnonneg s hs) (hupper s hs) (htotal s hs) i) t ht
  exact ⟨X,hX0,hpos,hdX⟩

theorem zero_supply_positive_global_solution (n : ℕ) (e : ℝ)
    (he : 0 ≤ e) (he' : e ≤ 1/50000)
    (s₀ : State) (x₀ : Fin n → ℝ) (r₀ : ℝ)
    (hs₀ : s₀.Positive) (hx₀ : ∀ i, 0 < x₀ i) (hr₀ : 0 < r₀) :
    ∃ Y : ℝ → State, ∃ x : ℝ → Fin n → ℝ, ∃ R : ℝ → ℝ,
      Y 0 = s₀ ∧ x 0 = x₀ ∧ R 0 = r₀ ∧ IsReservoirTrajectory n e 0 0 Y x R := by
  ...
  have hy₀ : ∀ i, 0 < y₀ i := by
    ...
    | inl i =>
      ...
      · exact hr₀
    | inr i => exact hx₀ i
  obtain ⟨Z,hZ0,hZp,hZd⟩ := zero_supply_positive_global_vector n e he he' y₀ hy₀
