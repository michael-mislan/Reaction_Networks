...
theorem killed_source_survival_bound (e : ℝ) (he : 0 ≤ e) (N : ℕ)
    (D : Finset Counts) (f : Point → ℝ) (hf : ∀ x, 0 ≤ f x)
    (hone : ∀ n ∈ D, 1 ≤ f (concentration N n)) (k : ℝ)
    (hgen : ∀ n ∈ D, generator e N f (concentration N n) ≤
      -k*f (concentration N n))
    (q t : ℝ≥0) (hq : 0 < (q : ℝ)) (hkq : k ≤ q)
    (hclock : ∀ s, (stoppedCountModel e he N D).total s ≤ q)
    (n : {n : Counts // n ∈ D}) :
    ((stoppedCountModel e he N D).uniformize q hq hclock).poissonized (q*t)
      (FiniteKernel.eventIndicator {s | s ≠ none}) (some n) ≤
        Real.exp (-k*(t : ℝ))*f (concentration N n.val) := by
  classical
  let M := stoppedCountModel e he N D
  let P := M.uniformize q hq hclock
  let W := stoppedObservable N D f 0
  have hG (s) : M.generator W s ≤ -k*W s := by
    cases s with
    | none => simp [M, W, stoppedObservable, stopped_none_generator]
    | some m => exact (stopped_generator_le e he N D f 0
        (fun _ _ _ _ _ => hf _) m).trans (hgen m.val m.property)
  have hI (s) : FiniteKernel.eventIndicator {s | s ≠ none} s ≤ W s := by
    cases s with
    | none => simp [FiniteKernel.eventIndicator, W, stoppedObservable]
    | some m => simpa [FiniteKernel.eventIndicator, W, stoppedObservable] using hone m.val m.property
  let r : ℝ := 1-k/(q : ℝ)
