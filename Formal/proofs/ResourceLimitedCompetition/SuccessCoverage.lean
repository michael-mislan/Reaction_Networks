import proofs.ResourceLimitedCompetition.NutrientOdds
...
def competitionSuccessSet (N h l : ℕ) (zL zH : ℝ) :
...
def competitionFailureSet (N h l : ℕ) (zL zH : ℝ) :
  ...
  ![outerFailureSet N (h+l) zL zH _, divisionFailureSet N (h+l) zL zH _,
    ...
    oddsExceptionalSet N (N*h) (N*l) _, ancestryBelowSet N N true _,
    ancestryBelowSet N N false _, unflaggedTerminalSet N (h+l) zL zH _]

theorem competition_failure_coverage (N h l : ℕ) (hN : 0 < N) (hh : 0 < h) (hl : 0 < l)
    (zL zH : ℝ)
    (hzL : zL ∈ Icc (99579401232/100000000000 : ℝ) (99579401233/100000000000))
    (hzH : zH ∈ Icc (297636724376/100000000000 : ℝ) (297636724377/100000000000))
    (x : StoppedPopulation (activeDomain N (h+l) zL zH))
    (hx : x ∉ competitionSuccessSet N h l zL zH) : ∃ i, x ∈ competitionFailureSet N h l zL zH i := by
  classical
  cases x with
  | inl s => exact ⟨3,⟨s,rfl⟩⟩
  | inr e =>
    ...
    | nutrient =>
      ...
      · by_cases hL : N ≤ ancestralMembrane false (eventOutcome N e).live
        · have hHp := hN.trans_le hH
          ...
          · exact ⟨4,nutrient_bad_odds_exceptional N h l hN hh hl zL zH e hr hHp hLp hb⟩
          · have hs := activeDomain_safe N (h+l) zL zH e.1.val e.1.property
            ...
            have hfreq : Real.exp (1/10)*(h : ℝ)/(l : ℝ) ≤
