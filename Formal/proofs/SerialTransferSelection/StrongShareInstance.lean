...
theorem strong_manyCycle_share_instance (N B K : ℕ)
      ...
      ∃ (JB JR : ℕ) (qb qr : NNReal) (hqb : 0 < (qb : ℝ)) (hqr : 0 < (qr : ℝ))
        ...
        confidence ≤
          (sourceManyCycleLaw N (2*B) JB JR (by omega) (by omega) zL zH hzL hzH
            ...
            (FiniteKernel.eventIndicator {h | manyMeasuredEvent N (2*B) zL zH (1/50) K s.val h ∧ historyGood (shareHistoryStep N (2*B) zL zH shareFloor (1/50)) K 0 (some s) h}) := by
  ...
  · have hmono := finiteLaw_event_mono_many
      ...
      {h | manyMeasuredEvent N (2*B) zL zH (1/50) K s.val h ∧ historyGood (shareHistoryStep N (2*B) zL zH shareFloor (1/50)) K 0 (some s) h}
      (fun h hh => ⟨shareHistory_measured N (2*B) (by omega) zL zH hzL hzH
        shareFloor (1/50) K s hnew hpos h hh,hh⟩)
