...
def newbornCountGain (N M : ℕ) (zL zH : ℝ) (s : PopulationState)
    (y : ReadyPopulation N M zL zH) : Prop :=
  (∀ b, 0 < ancestralCount b y.val.live) ∧
    2*cycleSizeGain (1/50)-Real.log 2 < countLogOdds y.val-countLogOdds s

theorem sourceTwoCycle_newborn_probability (N M JB JR : ℕ) (hN : 1 ≤ N) (hM : 0 < M) (zL zH : ℝ)
    ...
    1-(sourceCycleError N M JB JR qb qr t (1/2) (1/50)+sourceCycleError N M JB JR qb qr t (1/100) (1/50)) ≤
      (sourceTwoCycleLaw N M JB JR hN hM zL zH hzL hzH qr hqr hclockr γ hγ qb t hqb hclockb s).expect
        (FiniteKernel.eventIndicator (successfulOutput (newbornCountGain N M zL zH s.val))) := by
  ...
  have hc := newborn_endpoint_count_gain N (by omega) s.val y.val
    (readyPopulation_ready N M zL zH s).2.2.2.1
    (readyPopulation_ready N M zL zH y).2.2.2.1 hnew hcs hy.1 _ hy.2
  exact ⟨hy.1,hc⟩
