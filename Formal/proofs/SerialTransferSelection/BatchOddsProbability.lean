...
open scoped NNReal
noncomputable local instance PhaseOddsDecidableEq (D : Finset ResourceLimitedCompetition.PopulationState) : DecidableEq (ResourceLimitedCompetition.StoppedPopulation D) := Classical.decEq _
open ResourceLimitedCompetition HeritableCompositions FiniteCopy Set

theorem phase_global_odds_generator (γ : ℝ) (hγ : 0 ≤ γ) (Ω N M W0 H0 L0 : ℕ)
    (hN : 1000 ≤ N) (zL zH : ℝ)
  ...
  | inl s =>
    ...
    have hb : 0 ≤ resourceCoefficient γ s.val.resource Ω := by
      ...
      positivity
    rw [phase_ancestral_generator_binding]
...
theorem phase_global_odds_probability (γ : ℝ) (hγ : 0 ≤ γ) (Ω N M W0 : ℕ)
    (hN : 1000 ≤ N) (zL zH : ℝ)
    (hzL : zL ∈ Icc (99579401232/100000000000 : ℝ) (99579401233/100000000000))
    (hzH : zH ∈ Icc (297636724376/100000000000 : ℝ) (297636724377/100000000000))
    (q t : ℝ≥0) (hq : 0 < (q : ℝ))
    ...
    ((phaseStoppedModel γ hγ Ω N W0 zL zH (phaseActiveDomain N M W0 zL zH)).uniformize q hq hbound).poissonized
      (q*t) (FiniteKernel.eventIndicator (oddsExceptionalSet N (ancestralMembrane true s.val.live)
        (ancestralMembrane false s.val.live) (phaseActiveDomain N M W0 zL zH))) (.inl s) ≤
      Real.exp (-(19*(N : ℝ)/500000)) := by
  classical
  have h := (phaseStoppedModel γ hγ Ω N W0 zL zH (phaseActiveDomain N M W0 zL zH)).uniformized_event_bound
    q t hq hbound
    ...
    (fun _ hx => hx)
    (phase_global_odds_generator γ hγ Ω N M W0 _ _ hN zL zH hzL hzH) (.inl s)
