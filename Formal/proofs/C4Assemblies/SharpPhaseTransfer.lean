...
import proofs.C4Assemblies.SharpPhaseBounds
namespace C4Assemblies
noncomputable section
open Set ProductiveRecovery
variable {ι : Type*} [Fintype ι]

theorem sharp_backward_phase_local (a r d : ℝ) (ha : 0 ≤ a) (c : State) (hc : Nonneg c)
    (hr : 19 ≤ r) (hr' : r ≤ 21) (hd : 0 ≤ d) (hd' : d ≤ 1/25)
    (hA : A c ≤ 11/10) (hB : B c ≤ 11/10) :
    0 ≤ 48*phaseWeight a c+phaseWeight a (field r d c)-20*c 3-1160*a*c 4-38*c 5 := by
  obtain ⟨hx,hc1,hc2,hz⟩ := sharp_phase_comparison r d c hc hr hr' hd hd' hA hB
...
theorem sharp_backward_phase_deriv (X : ℝ → State) (V : State) (h t : ℝ)
    (hX : HasDerivAt X V t) :
    HasDerivAt (fun s => Real.exp (48*s)*phaseWeight (h-s) (X s))
      (Real.exp (48*t)*(48*phaseWeight (h-t) (X t)+phaseWeight (h-t) V-
...
theorem sharp_phase_minimum_propagation (k : ι → ι → ℝ) (hk : ∀ i j, 0 ≤ k i j)
  ...
  let W : ℝ → ι → ℝ := fun t i => Real.exp (48*t)*phaseWeight (h-t) (X t i)
  let V : ℝ → ι → ℝ := fun t i => Real.exp (48*t)*
    (48*phaseWeight (h-t) (X t i)+phaseWeight (h-t) (assemblyField k r d (X t) i)-
      20*X t i 3-1160*(h-t)*X t i 4-38*X t i 5)
  have hder : ∀ t ∈ Icc 0 h, ∀ i, HasDerivAt (fun s => W s i) (V t i) t := by
    intro t ht i
    exact sharp_backward_phase_deriv (fun s => X s i) _ h t (hasDerivAt_pi.1 (hX t ht) i)
  have hinit : ∀ i, L ≤ W 0 i := by simpa [W] using h0
  have hv : ∀ t ∈ Ico 0 h, ∀ i, diffusion k (W t) i ≤ V t i := by
    intro t ht i
    have hh' := sharp_backward_phase_local (h-t) (r i) (d i) (by linarith [ht.2])
