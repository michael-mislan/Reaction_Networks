import proofs.C4Assemblies.Examples
import proofs.C4Assemblies.SharpFreeProduct
import proofs.C4Assemblies.ReadySynthesis
...
theorem sharp_conservative_assembly_operation (P : Parameters ι) (p0 : ι → Intervention)
  ...
  have hf (n : ℕ) := sharp_assembly_routine_free_export P
...
def sharp_path_operation := sharp_conservative_assembly_operation pathParameters
  (fun _ => fixedPulse) (fun _ _ _ _ => fixedPulse) heterogeneousReady
  (fun i => strong_admitted _ (heterogeneous_ready i))

def sharp_ring_operation := sharp_conservative_assembly_operation ringParameters
  (fun _ => fixedPulse) (fun _ _ _ _ => fixedPulse) heterogeneousReady
  (fun i => strong_admitted _ (heterogeneous_ready i))
