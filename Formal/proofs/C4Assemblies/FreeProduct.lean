...
theorem assembly_free_phase_floor (P : Parameters ι) (X : ℝ → Assembly ι)
  ...
  have hcoef (i : ι) : (116/343)*b ≤ phaseWeight (1/35) (Z 0 i) := by
    have hi := hY i
    have h2 := hn t ht i 2
    have h3 := hn t ht i 3
    have h5 := hn t ht i 5
    ...
    norm_num
    linarith
  have hp := phase_minimum_propagation P.k P.exchange_nonneg P.r P.d P.r_lower P.r_upper
    (fun i => by linarith [P.d_lower i]) P.d_upper Z (1/35) ((116/343)*b) (by norm_num)
    (fun s hs => hZ s hs.1)
  ...
  have hh := hp i
  norm_num [Z] at hh
  have hx := hn (t+1/35) (by linarith) i 2
  have hmul := mul_le_mul_of_nonneg_right he.le hx
  nlinarith

theorem assembly_routine_free_export (P : Parameters ι) (p : ι → Intervention)
    (c : Assembly ι) (hc : AssemblyReady c) (X : ℝ → Assembly ι)
    (h0 : X 0 = assemblyPulse p c) (hn : ∀ t, 0 ≤ t → ∀ i, Nonneg (X t i))
    (hX : ∀ t, 0 ≤ t → HasDerivAt X (assemblyField P.k P.r P.d (X t)) t) :
    ∀ i, (∀ t ∈ Icc (3:ℝ) 4, 1/540 ≤ X t i 2) ∧
      1/540 ≤ ∫ t in (3:ℝ)..4, X t i 2 := by
  ...
  have hfloor (t : ℝ) (ht : t ∈ Icc (3:ℝ) 4) (i : ι) : 1/540 ≤ X t i 2 := by
    have h := assembly_free_phase_floor P X hn hX
      (fun s hs j => ⟨(hcor s hs j).1.2,(hcor s hs j).2.2⟩) (t-1/35) (1/20)
      (by linarith [ht.1]) (hY (t-1/35) (by linarith [ht.1])) i
    norm_num at h
  ...
  have h := intervalIntegral.integral_mono_on (a := (3:ℝ)) (b := 4) (by norm_num)
    (intervalIntegrable_const : IntervalIntegrable (fun _ : ℝ => (1/540:ℝ)) volume 3 4)
