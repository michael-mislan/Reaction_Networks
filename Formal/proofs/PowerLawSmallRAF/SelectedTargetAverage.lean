import proofs.PowerLawSmallRAF.TargetUnionSourceDock
...
abbrev RetainedSourceLowRows (n : Nat) (d : SourceDegreeConfig n) :=
  SourceLowOwnerGroup n d → Finset (Reaction n)

theorem source_target_failure_mass_bounds {q : ℝ} (hq0 : 0 ≤ q) (hq1 : q ≤ 1)
    (n L : Nat) (W : Finset LigationWord) (T : Finset (Reaction n) → Finset (Reaction n)) :
    0 ≤ sourceLigationTargetsFailureMass q n L W T ∧
      sourceLigationTargetsFailureMass q n L W T ≤ 1 := by
  unfold sourceLigationTargetsFailureMass
  constructor
...
def targetSetAdmissible (n m : Nat) (K : ℝ) (W : Finset LigationWord) : Prop :=
  (W.card : ℝ) ≤ K ∧ ∀ w ∈ W, m ≤ w.length ∧ w.length ≤ n
...
def sourceSelectedTargetMass (n m : Nat) (K : ℝ)
    ...
    ∑ A : RetainedSourceLowRows n d,
      bernoulliRowsWeight (fun x => sourceBandBernoulliParameter n d x.val) A *
        if targetSetAdmissible n m K (W d A) then
          sourceLigationTargetsFailureMass (sourceHighUnionParameter n d) n (targetNucleusLength n) (W d A) (T d A)
        else 0
...
theorem sourceSelectedTargetMass_bound (n m : Nat) (K E : ℝ) (hn : 4 ≤ n) (hE : 0 ≤ E)
    (W : (d : SourceDegreeConfig n) → RetainedSourceLowRows n d → Finset LigationWord)
    ...
    (hbound : ∀ q : ℝ, targetIntensity n ≤ q → q ≤ 1 → ∀ V : Finset LigationWord,
      ...
      (∀ H, H ⊆ P H) → sourceLigationTargetsFailureMass q n (targetNucleusLength n) V P ≤ E) :
    0 ≤ sourceSelectedTargetMass n m K W T ∧
      sourceSelectedTargetMass n m K W T ≤ sourceHighUnionFailureMass n+E := by
  ...
  have hp (d : SourceDegreeConfig n) (A : RetainedSourceLowRows n d) :
      0 ≤ (if targetSetAdmissible n m K (W d A) then
        sourceLigationTargetsFailureMass (sourceHighUnionParameter n d) n (targetNucleusLength n) (W d A) (T d A) else 0) ∧
      (if targetSetAdmissible n m K (W d A) then
        sourceLigationTargetsFailureMass (sourceHighUnionParameter n d) n (targetNucleusLength n) (W d A) (T d A) else 0) ≤
      (if sourceHighUnionParameter n d < targetIntensity n then 1 else 0)+E := by
    have hq := (sourceBandUnionParameter_bounds n hn d).2
    have hm := source_target_failure_mass_bounds hq.1 hq.2 n (targetNucleusLength n) (W d A) (T d A)
    constructor
    ...
    · by_cases hv : targetSetAdmissible n m K (W d A)
      · rw [if_pos hv]
        ...
        · rw [if_neg hb,zero_add]
          exact hbound _ (le_of_not_gt hb) hq.2 _ hv _ (hT d A)
      · rw [if_neg hv]
  ...
  · unfold sourceSelectedTargetMass
    calc
...
theorem sourceSelectedHighTargets_tendsto_zero
    ...
    (hT : ∀ n d A H, H ⊆ T n d A H) :
    Tendsto (fun n => sourceSelectedTargetMass n (n-2*shrinkingBandWidth n)
  ...
  have he : ∀ᶠ n : Nat in atTop, 0 ≤ sourceSelectedTargetMass n (n-2*shrinkingBandWidth n)
      ((n : ℝ)^3*(2 : ℝ)^(shrinkingBandWidth n)) (W n) (T n) ∧
      sourceSelectedTargetMass n (n-2*shrinkingBandWidth n)
      ((n : ℝ)^3*(2 : ℝ)^(shrinkingBandWidth n)) (W n) (T n) ≤ sourceHighUnionFailureMass n+highTargetError n := by
    filter_upwards [eventually_ge_atTop 4,source_high_target_uniform_bound] with n hn hu
    apply sourceSelectedTargetMass_bound _ _ _ _ hn (by unfold highTargetError; positivity) _ _ (hT n)
    intro q hpq hq V hV P hP
    exact hu q hpq hq V hV.1 hV.2 P hP
  exact tendsto_of_tendsto_of_tendsto_of_le_of_le' tendsto_const_nhds hb
    (he.mono fun _ h => h.1) (he.mono fun _ h => h.2)

theorem sourceSelectedLowTargets_tendsto_zero
    (W : (n : Nat) → (d : SourceDegreeConfig n) → RetainedSourceLowRows n d → Finset LigationWord)
    (T : (n : Nat) → (d : SourceDegreeConfig n) → RetainedSourceLowRows n d → Finset (Reaction n) → Finset (Reaction n))
    (hT : ∀ n d A H, H ⊆ T n d A H) :
    Tendsto (fun n => sourceSelectedTargetMass n (n/200)
  ...
  simp only [add_zero] at hb
  have he : ∀ᶠ n : Nat in atTop, 0 ≤ sourceSelectedTargetMass n (n/200)
      ((2 : ℝ)^(targetNucleusLength n+1)) (W n) (T n) ∧
      sourceSelectedTargetMass n (n/200) ((2 : ℝ)^(targetNucleusLength n+1)) (W n) (T n) ≤
        sourceHighUnionFailureMass n+lowTargetError n := by
    filter_upwards [eventually_ge_atTop 4,source_low_target_uniform_bound] with n hn hu
    apply sourceSelectedTargetMass_bound _ _ _ _ hn (by unfold lowTargetError; positivity) _ _ (hT n)
    intro q hpq hq V hV P hP
    exact hu q hpq hq V hV.1 hV.2 P hP
  exact tendsto_of_tendsto_of_tendsto_of_le_of_le' tendsto_const_nhds hb
    (he.mono fun _ h => h.1) (he.mono fun _ h => h.2)
