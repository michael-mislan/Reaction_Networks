...
import proofs.PowerLawSmallRAF.BoundedTargetsProbability
...
theorem source_bounded_union_bound_with_intensity_floor
    ...
    sourceBoundedTargetsFailureMass q n L W T ≤
      K*(Real.exp (-p*(m : ℝ)/2)+2*(n : ℝ)^2*Real.exp (-p*(L : ℝ)^2/32)) := by
  have hb := source_bounded_targets_failure_bound (hp.trans hpq) hq n L m W hW T hL hLm
...
theorem source_bounded_high_uniform_bound :
    ∀ᶠ n : Nat in atTop, ∀ q : ℝ, targetIntensity n ≤ q → q ≤ 1 →
      ...
      ∀ T : Finset (Reaction n) → Finset (Reaction n),
      sourceBoundedTargetsFailureMass q n (targetNucleusLength n) W T ≤ highTargetError n := by
  filter_upwards [targetUnion_eventual_conditions] with n hn
  intro q hpq hq W hK hW T
  exact source_bounded_union_bound_with_intensity_floor hn.1 hpq hq n (targetNucleusLength n)
    (n-2*shrinkingBandWidth n) W hK hW T hn.2.2.1 hn.2.2.2.2.1 hn.2.2.2.2.2

theorem source_bounded_low_uniform_bound :
    ∀ᶠ n : Nat in atTop, ∀ q : ℝ, targetIntensity n ≤ q → q ≤ 1 →
      ∀ W : Finset LigationWord,
      (W.card : ℝ) ≤ (2 : ℝ)^(targetNucleusLength n+1) →
      (∀ w ∈ W, n/200 ≤ w.length ∧ w.length ≤ n) →
      ∀ T : Finset (Reaction n) → Finset (Reaction n),
      sourceBoundedTargetsFailureMass q n (targetNucleusLength n) W T ≤ lowTargetError n := by
  filter_upwards [targetUnion_eventual_conditions] with n hn
  intro q hpq hq W hK hW T
  exact source_bounded_union_bound_with_intensity_floor hn.1 hpq hq n (targetNucleusLength n)
    (n/200) W hK hW T hn.2.2.1 hn.2.2.2.1 hn.2.2.2.2.2
