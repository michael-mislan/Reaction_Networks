import proofs.CompositionalMemory.GenericArchitectureGeneration
import proofs.CompositionalMemory.GenericSixErrorBudget

namespace CompositionalMemory
open FiniteCopy

/-- Exponential accuracy of the actual generation law, with an explicit
prefactor independent of module count and copy number. -/
theorem general_architecture_generation_exponential {k d : ℕ} {ι : Type*} [Fintype ι]
    ...
    (generalGenerationLaw hk next rate hrate N C hN Q center c radius birth outer parent recover
      hc hradius hbirth.le houter hgap hcenter hcoerc q₀ t₀ q₁ t₁ hq₀ hq₁ hclock₀ hclock₁ n).mass none ≤
      (k:ℝ)*(7+2*(d:ℝ)+(lam*rho/2)*((t₀:ℝ)/(outer-birth)+(t₁:ℝ)/(parent-recover)))*Real.exp (-cgen*N) := by
  classical
  intro next rate hrate domain₀ domain₁ q₀ t₀ q₁ t₁ hq₀ hq₁ hd₀ ht₀ hearly ht₁ hd₁ hclock₀ hclock₁ n
  ...
  have hraw := general_architecture_generation_failure hk N C hN hC consume produce coeff z γ w hcoeff hγ hw
    ...
    hsres hscouple habsorb hforce hcopy hquality hz parent lo rpart δ hbirth.le hgap hlo hrpart hδ
    hcoerc hsize hmargin hzlo q₀ t₀ q₁ t₁ hq₀ hq₁ hd₀ ht₀ hearly ht₁ hd₁ hclock₀ hclock₁ n
  have hcoef := recovery_ceiling_coefficient α N lam rho
    (2*L*D/N+2*L*γ*Z*(U+1)+4*L*Z*κ)
    (A*R+L*γ*Z*(U+1)^2+2*L*Z*κ)
    (2*A*R^2+2*L^2*γ*Z*(U+1)^4+4*L^2*Z*κ)
    hα.le (Nat.cast_nonneg N) hlam hforce hcopy
  have hceiling : ceiling ≤ (α*(lam*rho)*N/4)*Real.exp (α*N*(M*rho)) := by
    calc
      _ ≤ (α*lam*N*rho/4)*Real.exp (α*N*(M*rho)) :=
  ...
  exact generation_six_error_bound _ k d N α (lam*rho) t₀ t₁ (M*rho) recover parent birth outer δ C ceiling cgen
    hkr (Nat.cast_nonneg d) (Nat.cast_nonneg N) hα (mul_nonneg hlam.le hrho.le)
    t₀.coe_nonneg t₁.coe_nonneg hcore hgap hparentbirth hbirth hceiling hc₀ hc₁ hcr hce hcl hcp
    (by simpa only [Nat.cast_mul] using hraw)
