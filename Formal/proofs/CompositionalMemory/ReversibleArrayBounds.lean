...
theorem all_array_bounds (N : Nat) (data : Nat → Array Int)
  ...
  · have hm : N+j.val=2*N := by omega
    rw [hm] at hi ⊢
    have hr := ControlledRows.checked_terminal_rewards N (data (2*N)) ht i hi
    have hS : (0 : ℝ) < (precisionScale : ℝ) := by norm_num [precisionScale,ControlledRows.precisionScale]
    constructor
    · intro col
      have hh : (value (data (2*N)) i col.val : ℝ)/(precisionScale : ℝ) ≤ 1 := by
        fin_cases col
        · exact hr.2.1.trans (ControlledRows.terminalReward_bounds N _ _ 0).2
        · exact hr.2.2.2.1.trans (ControlledRows.terminalReward_bounds N _ _ 1).2
      have hh' := (div_le_one hS).mp hh
      exact_mod_cast hh'
    · have hw := (of_decide_eq_true (ControlledRows.terminal_checked_row N (data (2*N)) ht i hi)).2.2.2.2
      rw [hw]
      exact le_rfl
