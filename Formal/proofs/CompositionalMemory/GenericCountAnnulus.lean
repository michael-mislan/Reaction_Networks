...
/-- Negative exponential drift outside the recovery core for the literal
count architecture, derived from macroscopic mass-action dissipation. -/
theorem general_count_annular_generator {k d : ℕ} {ι : Type*} [Fintype ι]
    ...
    ∀ (Q : (Fin d → ℝ) →ₗ[ℝ] (Fin d → ℝ) →ₗ[ℝ] ℝ)
      (α N κ lam r rho A P R U Z L D : ℝ),
    (∀ x q, Q x q=Q q x) →
    ...
    reactionGenerator (generalGlobalNext consume produce z)
      ...
      (-α*lam*N*rho/4)*Real.exp (α*N*Q y y) := by
  classical
  intro v u y ν Q α N κ lam r rho A P R U Z L D hQ hN hNv hP hα hlam hγ hR
    hU hZ hL hr hc hA hD hcoord hcross hquad hd hw hz hrow hop hy hu
    hsres hscouple habsorb hrho hforce hcopy
  ...
  have hb : ‖b‖ ≤ 1 := by
    ...
    dsimp [b]
    split_ifs <;> norm_num
  have hlocal := generic_local_annular_observable hk i Q hQ y u b ν a
    (fun j => generalConcentration s j z) (w i) α N v γ κ lam r rho A P R
