import proofs.CompositionalMemory.GenericLocalAnnulus
import proofs.CompositionalMemory.GenericExponentialObservable
...
theorem generic_local_annular_observable {V ι : Type*}
  ...
  have h := generic_local_annular_generator hk i Q hQ y u b ν a z w
    α N v γ κ lam r rho A P R F U Z L hN hNv hP hα hlam hγ hR hU hZ hL hr
    ha hA hcross hquad hd hw hz hrow hop hy hu hb hsres hscouple habsorb hrho hforce hcopy
  exact exponential_observable_of_ratio (localChannelRate a γ v z w i)
    (fun j => Q (y+localChannelJump ν u b v i j) (y+localChannelJump ν u b v i j))
    (Q y y) (α*N) (-α*lam*N*rho/4) h
