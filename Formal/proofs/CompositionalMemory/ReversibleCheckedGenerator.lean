import proofs.CompositionalMemory.ReversibleLocalGenerator
import proofs.CompositionalMemory.ReversibleCheckedRows

namespace CompositionalMemory
open FiniteIntegerRows ControlledRows

theorem reversible_checked_local_bounds (N : Nat) (hN : 0 < N) (ε : ℝ)
    ...
    -(1/1000000 : ℝ) ≤ reversibleControlledLocal N ε data s k 0 ∧
    -(1/1000000 : ℝ) ≤ reversibleControlledLocal N ε data s k 1 ∧
    reversibleControlledLocal N ε data s k 2 ≤ -(5/8 : ℝ)*coupledCurrent N data s k 2+1/50000 := by
  let m := N+s.1.val
  ...
  have hrow := ReversibleRows.checked_row m (data m) (data (m+1))
  ...
  have hc := coupled_control_ranges m hm (s.2 other).1.val (s.2 other).2.val
    ...
    ε hε hε1
  have hl := ReversibleRows.checked_literal_bounds m hm (data m) (data (m+1)) i hrow
    ((s.2 other).1.val/(16*(m : ℝ))) (5*ε*(s.2 other).2.val/(2*(m : ℝ)))
    hc.1 hc.2.1 hc.2.2.1 hc.2.2.2
  simp only [reversible_local_controlled_literal N hN ε data s hs]
  refine ⟨hl.1,hl.2.1,?_⟩
  convert hl.2.2 using 1
  dsimp [coupledCurrent,m,i,ReversibleRows.precisionScale]
  ring
