...
theorem checked_value_upper (m : Nat) (v next : Array Int) (i : Nat)
  ...
  have hh : value v i col.val ≤ precisionScale := by
    ...
    · exact h.2.2.2.1
    · exact h.2.2.2.2.1
  apply (div_le_one (by norm_num [precisionScale,ControlledRows.precisionScale])).mpr
  exact_mod_cast hh
...
theorem checked_time_lower (m : Nat) (v next : Array Int) (i : Nat)
  ...
  apply (le_div_iff₀ (by norm_num [precisionScale,ControlledRows.precisionScale])).mpr
...
theorem checked_birth_bound (N : Nat) (v : Array Int) (hb : birthOK N v=true)
    ...
    (19911/20000 : ℝ) ≤ (value v i col.val : ℝ)/(precisionScale : ℝ)-
      (value v i 2 : ℝ)/(500000*(precisionScale : ℝ)) := by
  have h := of_decide_eq_true ((List.all_eq_true.mp hb) i (List.mem_range.mpr hi))
  have hh : 497775*precisionScale ≤ 500000*value v i col.val-value v i 2 := by
    fin_cases col
    · exact h.1 hnew
    · exact h.2 hnew
  have hr : (497775 : ℝ)*(precisionScale : ℝ) ≤
      500000*(value v i col.val : ℝ)-(value v i 2 : ℝ) := by exact_mod_cast hh
  norm_num [precisionScale,ControlledRows.precisionScale] at hr ⊢
  linarith
