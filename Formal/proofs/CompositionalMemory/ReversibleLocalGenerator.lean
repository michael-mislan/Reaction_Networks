import proofs.CompositionalMemory.ReversibleGeneratorEnvelope

namespace CompositionalMemory
open FiniteIntegerRows ControlledRows
set_option maxHeartbeats 400000

theorem reversible_local_controlled_literal (N : Nat) (hN : 0 < N) (ε : ℝ)
    (data : Nat → Array Int) (s : CoupledLiveState N) (hs : s.1.val < N)
    (k : Fin 2) (col : Nat) :
    reversibleControlledLocal N ε data s k col=
      ReversibleRows.literal (N+s.1.val) (data (N+s.1.val)) (data (N+s.1.val+1))
  ...
  have hcast (u : Fin 2) (hh : (a u).1 ≠ 0) :
    ...
    norm_num
  fin_cases k <;>
    simp only [reversibleControlledLocal,Fintype.sum_prod_type,Fin.sum_univ_succ] <;>
    norm_num [reversibleRate,reversibleProposal,coupledProposal,hj,Function.update,
      coupledCurrent,ReversibleRows.literal,ReversibleRows.precisionScale,hi,Nat.add_assoc] <;>
    simp only [hc]
  all_goals
