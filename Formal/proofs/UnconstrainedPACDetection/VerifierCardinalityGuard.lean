import proofs.UnconstrainedPACDetection.VerifierCardinalityData

namespace UnconstrainedPACDetection.VerifierCardinalityGuard
open Complexity Complexity.TM
open VerifierBufferedProduct (wordTape)
open VerifierVerdictAnd (one)
open VerifierAccumulate (markerDir)

abbrev body := VerifierFieldCardinality.machine
...
theorem accepted_hoare (inp₀ : Tape) (base : Fin 11 → Tape)
    (hi : inp₀.read ≠ .start) (hp : ∀ j, Parked (base j))
    (post : Complexity.TM.TapePred 11) (B : ℕ)
    (hbody : body.HoareTime
      (fun inp work out => inp = inp₀ ∧ work = base ∧ out = one .start true) post B) :
    machine.HoareTime
      (fun inp work out => inp = inp₀ ∧ work = base ∧ out = one .start true) post (B+2) := by
  rintro inp work out ⟨hin,hw,ho⟩
  subst inp; subst work; subst out
  obtain ⟨d,t,ht,hr,hh,hpost⟩ := hbody inp₀ base (one .start true) ⟨rfl,rfl,rfl⟩
...
end UnconstrainedPACDetection.VerifierCardinalityGuard
