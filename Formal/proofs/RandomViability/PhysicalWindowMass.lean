...
theorem physical_window_mass_from_noise {n : ℕ} (hn : 2 ≤ n)
    ...
    (T : ℝ) (hT : 100 < T) (z : ℕ → JumpState (Molecule n → ℕ) (PhysicalCountChannel n))
    ...
    : ∃ J K,prefixElapsed J (Preorder.frestrictLe J z) ≤ 1 ∧
      ...
      99*((19/10 : ℝ)-480*(1/500000000 : ℝ))-54 ≤
        640*physicalWindowNonfoodIntegral V z J K := by
  obtain ⟨J,K,hfirst,hfirstNext,hlast,hlastNext⟩ :=
    exists_time_window_indices z hh hd 1 100 (by norm_num) (by norm_num)
  ...
  have hfloor : ∀ i ≤ K,(1/3000000000000000000 : ℝ) ≤ ((z (J+i)).1 (reactionProduct r) : ℝ)/V := by
    ...
    have hnext : 1 < prefixElapsed (J+i+1) (Preorder.frestrictLe (J+i+1) z) :=
      hfirstNext.trans_le (hm (by omega))
  ...
  have hg := physical_window_potential_gain c V hV basal cat hb hcatCap hfood r
    hl hr hlen huw huz hwz hsel hbas hcat T hT z J K hfirst hfirstNext hlast hlastNext
