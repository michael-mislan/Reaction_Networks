...
def competitionFoodInitial (V : ℕ) (hV : 0 < (V:ℝ)) : CompetitionCounts V :=
  ⟨(foodInitial V,0),by
    norm_num [scaledPhaseValid,foodInitial,boxCounts,weightedCount,weighted]
    constructor
    · exact_mod_cast hV
    · intro _; positivity⟩

/-- Startup followed by m full unit windows on the continued chemical state. -/
def competitionStartedFailure (V : ℕ) (S : FiniteKernel (CompetitionCounts V))
    (P : FiniteKernel (CompetitionWindowState V (competitionWindowCap V)))
    (q : ℝ≥0) (m : ℕ) (X : CompetitionCounts V) : ℝ :=
  S.poissonized (q*500) (fun Z => competitionMonitorSteps P q m competitionOperatingFailure ((Z,0),true)) X

theorem competition_startup_windows (V q : ℕ) (eps k r delta : ℝ)
  ...
  let b : ℝ := (m:ℝ)*(4*resourceSource V+competitionReturnSource V)+(m:ℝ)*Real.exp (1/8-(13/1080000)*(V:ℝ))
  ...
  have hpt (Z : CompetitionCounts V) : competitionMonitorSteps P q m competitionOperatingFailure ((Z,0),true) ≤ f Z+g Z+h Z+b := by
    have hp := competition_operating_windows V eps k r delta hV heps heps1 hk hk1 hr hr1 hd hd1
      q (by exact_mod_cast hq) (by simpa only [NNReal.coe_natCast] using hclock)
      (by simpa only [NNReal.coe_natCast] using hw) m (Z,0)
    dsimp [b,f,g,h]
    convert hp using 1
    ring
  have hon (Z : CompetitionCounts V) : 0 ≤ competitionMonitorSteps P q m competitionOperatingFailure ((Z,0),true) := by
  ...
  rw [hgX] at hrg
  rw [hhX] at hrt
  change S.poissonized ((q:ℝ≥0)*500) (fun Z => competitionMonitorSteps P q m competitionOperatingFailure ((Z,0),true)) X ≤ _
  dsimp [b] at hm
  linarith
