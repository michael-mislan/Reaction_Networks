import proofs.RandomViability.MarkedRewardBounds
...
def censoredMarkedRewardPrefix {n : ℕ} (c : SourceMoleculeFibreConfig n) (V : NNReal)
  ...
  ∑ i : Fin K,censoredMarkedRewardCompensation c V basal cat reward T stop
    i (Preorder.frestrictLe (i : ℕ) z) (z ((i : ℕ)+1))

def signedMarkedRewardFluctuationBy {n : ℕ} (c : SourceMoleculeFibreConfig n) (V : NNReal)
    (basal : Reaction n → NNReal) (cat : Reaction n → Molecule n → NNReal)
    (reward : (Molecule n → ℕ) → PhysicalCountChannel n → ℝ) (T : ℝ)
    (stop : (k : ℕ) → (Finset.Iic k → JumpState (Molecule n → ℕ) (PhysicalCountChannel n)) → Prop)
    (θ δ : ℝ) (K : ℕ) (z : ℕ → JumpState (Molecule n → ℕ) (PhysicalCountChannel n)) : Prop :=
  ∃ j,j ≤ K ∧ δ ≤ θ*(∑ i : Fin j,censoredMarkedRewardCompensation c V basal cat reward T stop
    i (Preorder.frestrictLe (i : ℕ) z) (z ((i : ℕ)+1)))

variable {n : ℕ} [MeasurableSpace (PhysicalCountChannel n)]
  [MeasurableSingletonClass (PhysicalCountChannel n)]

theorem physical_signed_marked_reward_crossing_tail (hn : 4 ≤ n)
  ...
  let m := censoredMarkedRewardMultiplier c V basal cat reward θ T stop
  ...
  have hc := predictable_product_crossing_bound μ κ
    ...
    m (censoredMarkedRewardMultiplier_measurable c V basal cat reward θ T stop hstop)
    (physical_censored_marked_reward_mean hn c V hV basal cat reward θ T
      (fun N hM => marked_reward_tilt_generator c V basal cat reward N (hreward N) (hvariance N hM) θ hθ) stop) a K
  have hsub : W ∩ {z | signedMarkedRewardFluctuationBy c V basal cat reward T stop θ δ K z} ⊆
    ...
    refine ⟨j,hj,?_⟩
    dsimp only [m,a]
    rw [censored_marked_reward_product_exp]
...
theorem physical_signed_marked_reward_any_index_tail (hn : 4 ≤ n)
    (c : SourceMoleculeFibreConfig n) (V : NNReal) (hV : 0 < (V : ℝ))
    (basal : Reaction n → NNReal) (cat : Reaction n → Molecule n → NNReal)
    (N : Molecule n → ℕ)
    (reward : (Molecule n → ℕ) → PhysicalCountChannel n → ℝ)
  ...
  exact iSup_le (fun K => physical_signed_marked_reward_crossing_tail hn c V hV basal cat N
    reward hreward hvariance θ T hθ hT stop hstop δ K)

/-- Uniform two-sided marked-reward compensation bound; no union cost in jump count. -/
theorem physical_marked_reward_two_sided_tail (hn : 4 ≤ n)
    (c : SourceMoleculeFibreConfig n) (V : NNReal) (hV : 0 < (V : ℝ))
    (basal : Reaction n → NNReal) (cat : Reaction n → Molecule n → NNReal)
    (N : Molecule n → ℕ)
    (reward : (Molecule n → ℕ) → PhysicalCountChannel n → ℝ)
  ...
  let E := fun t : ℝ => ⋃ K,{z | signedMarkedRewardFluctuationBy c V basal cat reward T stop t (θ*δ) K z}
  ...
  have hp : μ (E θ) ≤ B :=
    (physical_signed_marked_reward_any_index_tail hn c V hV basal cat N reward hreward hvariance θ T
  ...
  have hsub : {z | ∃ K,δ ≤ |censoredMarkedRewardPrefix c V basal cat reward T stop z K|} ⊆ E θ ∪ E (-θ) := by
    intro z hz
    obtain ⟨K,hK⟩ := hz
