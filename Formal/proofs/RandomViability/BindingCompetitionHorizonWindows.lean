import proofs.RandomViability.BindingCompetitionFinalFraction
...
theorem competition_horizon_windows (V : ℕ) (eps k r delta : ℝ)
    ...
    competitionMonitorSteps P q m (competitionFinalFailure P q s) (X,true) ≤
      competitionNoEntry X.1+resourcePotential V (boxCounts (competitionCounts X.1))+
      competitionTrackedReturnPotential X.1+
      ((m:ℝ)+(s:ℝ))*(4*resourceSource V+competitionReturnSource V)+
  ...
  let b : ℝ := (s:ℝ)*(4*resourceSource V+competitionReturnSource V)
  ...
  have hsum (Z) : 0 ≤ f Z+g Z+h Z+competitionMonitorBad Z := add_nonneg (hfgh Z) (competitionMonitorBad_nonneg Z)
  have ho (Z : CompetitionMonitorState V (competitionWindowCap V)) : 0 ≤ competitionFinalFailure P q s Z := by
    apply P.poissonized_nonneg
    intro Y
    unfold competitionOperatingFailure FiniteKernel.eventIndicator
    split_ifs <;> norm_num
  have hm := competition_monitor_steps_mono P q (competitionFinalFailure P q s)
    (fun Z => f Z+g Z+h Z+competitionMonitorBad Z+b) ho
    (fun Z => add_nonneg (hsum Z) hbn)
    (competition_final_fraction_bound V (competitionWindowCap V) eps k r delta hV heps heps1 hk hk1 hr hr1 hd hd1 q s hq hbound) m (X,true)
  rw [competition_monitor_steps_add P q _ _ hsum (fun _ => hbn),competition_monitor_steps_const,
    competition_monitor_steps_add P q _ _ hfgh competitionMonitorBad_nonneg,
  ...
  change competitionMonitorSteps P q m (competitionFinalFailure P q s) (X,true) ≤ _
  ...
  dsimp [f,g,h,b] at hn hrb hrt hm
  linarith
