import proofs.AllIrrRAFCert.Hardness.Source
...
variable {k n : Nat} (bad : Pair k n → Bool)

theorem selector_foodGenerated (S : Finset (Rxn k n))
    (hS : ∀ r ∈ S, ∃ v, r = Rxn.selector v) : FoodGenerated (crs bad) S := by
  intro r hr
  obtain ⟨v,rfl⟩ := hS r hr
  exact ⟨0, Finset.Subset.rfl⟩

theorem guard_isRAF (i : Fin k) : IsRAF (crs bad) cat (guard (n := n) i) := by
  have hfg : FoodGenerated (crs bad) (guard (n := n) i) := by
...
theorem no_close_selector (S : Finset (Rxn k n))
    (hS : IsRAF (crs bad) cat S) (ht : Rxn.close ∉ S)
    {r : Rxn k n} (hr : r ∈ S) : ∃ v, r = Rxn.selector v := by
  ...
  | pairOK e => simp [cat] at hc

theorem no_close_next (S : Finset (Rxn k n))
    (hS : IsRAF (crs bad) cat S) (ht : Rxn.close ∉ S)
...
theorem no_close_contains_guard (S : Finset (Rxn k n))
    (hS : IsRAF (crs bad) cat S) (ht : Rxn.close ∉ S)
...
theorem guard_isIrrRAF (i : Fin k) :
    IsIrreducibleRAF (crs bad) cat (guard (n := n) i) := by
...
theorem irrRAF_without_close_eq_guard (S : Finset (Rxn k n))
...
theorem guards_injective : Function.Injective (guard (k := k) (n := n)) := by
  intro i j h
  have hi : Rxn.selector (i,0) ∈ guard (n := n) i := by simp
  rw [h] at hi
