"""Computer-assisted proof of a finite-amplitude periodic orbit of witness A and of its
orbital stability, by multiple shooting and the Newton--Kantorovich theorem.

    python validate_orbit.py 7/5            (about ten minutes)
...
def validate(rstr, m=256, q=None, order=12, rho=None):
    t_start = time.time()
    rv = sp.Rational(rstr); model = witness('A', rv); fm = FloatModel(model); fld = Field(model); d = fld.d
    # --- candidate orbit (floating point, not part of the proof)
    D = json.load(open(ROOT/'figures'/'numerical_data.json'))
    key = f'waveform_r{float(rv)}'
    if key in D:
        y0 = np.array(D[key]['y0']); T = D[key]['T']
    else:
        b = [b for b in D['branch'] if abs(b['r'] - float(rv)) < 1e-9][0]; T = b['period']
        J0 = fm.DF(0, np.zeros(d)); ev, vec = np.linalg.eig(J0); kk = np.argmax(ev.real)
        y0 = fm.integrate(0.3*np.real(vec[:, kk]/vec[8, kk]), 40000, rtol=1e-10, atol=1e-13).y[:, -1]
    y0, T, Vmono, res = fm.periodic_orbit(y0, T, tol=1e-13)
    ...
    H = T/m; nodes = [y0]
    for l in range(m - 1):
        nodes.append(fm.integrate(nodes[-1], H, rtol=1e-13, atol=1e-15).y[:, -1])
    ...
    out = dict(r=rstr, evidence='binary64 interval arithmetic with outward rounding; Newton-Kantorovich on multiple shooting',
               ...
               node_values_float=Ybar.tolist(), phase_normal=phase.tolist(), gershgorin=gersh,
...
if __name__ == '__main__':
    validate(sys.argv[1] if len(sys.argv) > 1 else '7/5')
